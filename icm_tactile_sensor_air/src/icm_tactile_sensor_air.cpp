...
//        cout << "4: number of columns (int)" << endl;
        cout << "4: touch increment value (float)" << endl;
   }
    else
    {
...
//    int numOfColumns = std::stoi(argv[4]); 		// number of columns (short side)
    int numOfColumns = 1;
    float touchInc = std::stof(argv[4])/1000;			// touch increment value divided by 1000 to scale it to millimeters

    //float touchInc = 0;

    ros::init(argc, argv, "icm_tactile_sensor_air");
    ...
    ros::Publisher touchState_pub = node_handle.advertise<std_msgs::String>("touch_state", 1000);
...
//    move_group.setMaxVelocityScalingFactor(MAX_VEL_SCALE_FACTOR);
   ...
   // targetJoints["shoulder_pan_joint"] = -9.94*3.1416/180;
  //  targetJoints["shoulder_lift_joint"] = -31.43*3.1416/180;
   // targetJoints["elbow_joint"] = 35.81*3.1416/180;
   // targetJoints["wrist_1_joint"] = -96.39*3.1416/180;
   // targetJoints["wrist_2_joint"] = -93.97*3.1416/180;
    //targetJoints["wrist_3_joint"] = 3.01*3.1416/180; 

    // Modified for the new design method
    std::map<std::string, double> targetJoints;
    targetJoints["shoulder_pan_joint"] = -8.52*3.1416/180;
    targetJoints["shoulder_lift_joint"] = -30.55*3.1416/180;
    targetJoints["elbow_joint"] = 25.26*3.1416/180;
    targetJoints["wrist_1_joint"] = -84.08*3.1416/180;
    targetJoints["wrist_2_joint"] = -90.07*3.1416/180;
    targetJoints["wrist_3_joint"] = 89.34*3.1416/180; 
...
//    visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to continue with the next action");
    ...
    //targetJoints["shoulder_pan_joint"] = -20.49*3.1416/180;	// (deg*PI/180)
    //targetJoints["shoulder_lift_joint"] = -28.61*3.1416/180;
    //targetJoints["elbow_joint"] = 62.88*3.1416/180;
    //targetJoints["wrist_1_joint"] = -125.28*3.1416/180;
   // targetJoints["wrist_2_joint"] = -90.29*3.1416/180;
   // targetJoints["wrist_3_joint"] = 0.8*3.1416/180;

        //Modified for the new design method
    targetJoints["shoulder_pan_joint"] = -8.5*3.1416/180;	// (deg*PI/180)
    targetJoints["shoulder_lift_joint"] = -30.85*3.1416/180;
    targetJoints["elbow_joint"] = 54.40*3.1416/180;
    targetJoints["wrist_1_joint"] = -112.93*3.1416/180;
    targetJoints["wrist_2_joint"] = -90.07*3.1416/180;
    targetJoints["wrist_3_joint"] = 0.04*3.1416/180;


    // touch on the PCB tests
    move_group.setJointValueTarget(targetJoints);
...
//    float robotTouchZ = 0.015 + touchInc;	// 10mm it was 16mm (0.016)
    ...
    float yPosition [3] = { 0.0, 0.0, 0.00005};
    ...
    geometry_msgs::Pose homeYPosition = move_group.getCurrentPose().pose;
...
//            if (myTextFile.is_open())
			if (myfilePressure.is_open() && myfileTemperature.is_open() && myfileLoadCell.is_open() && myfileKalmanX.is_open() && myfileKalmanX.is_open() && myfileAccelerometer.is_open() && myfileGyro.is_open() && myfileForceTorque.is_open())
				ready_to_write = true;
			else
...
//            std::cout << "[X, Y, Z] = " << xPosition[i] << ", " << yPosition[i] << ", " << zPosition[i] << std::endl;
            ...
            if( i == 0 )
            {
                ...
                // This line is to keep Y axis fixed for this specific experiment.
                // This line needs to be deleted or modified if data collected from a matrix pattern is required
//            	target_pose3.position.y = homeYPosition.position.y;     
            }
            ...
            std_msgs::String msgTouchState;

            std::stringstream ss;
            ss << "ready";
            msgTouchState.data = ss.str();

            touchState_pub.publish(msgTouchState);
            ...
            if ( i == 2 )
                sleep(1.0); // 0.5 before
            else
                sleep(1.5); // 2.0 before
