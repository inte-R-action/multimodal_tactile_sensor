...
void DistanceValueCallback(const std_msgs::Float64::ConstPtr& msg)
{
  ...
  // ROS_INFO("I heard: [%f]", proximity);
       ...
       if (proximity < 300.0){
           velocity_constant = 0.10;  
        }
        else {
           velocity_constant = 0.28;  
        }
...
ros::Timer timer = node_handle.createTimer(ros::Duration(0.01),
  [&](const ros::TimerEvent&) {    
       if (proximity < 300){
            set_speed_frac.request.speed_slider_fraction = (proximity - 40) / 1000; // change velocity
          //  set_speed_frac.request.speed_slider_fraction = veocity_constant; // change velocity
            client.call(set_speed_frac);       
             cout << "velocity constant:" << velocity_constant << endl;    
        }
        else {
         set_speed_frac.request.speed_slider_fraction = 0.28; // change velocity
        // set_speed_frac.request.speed_slider_fraction = veocity_constant; // change velocity
        client.call(set_speed_frac);   
         cout << "velocity constant:" << velocity_constant << endl;
