...
void distance_value(const std_msgs::Float32::ConstPtr& msg)
{
 // ROS_INFO("I heard: [%f]", msg->data);
		proximity = msg->data;
       if (proximity < 300.0){
           velocity_constant = 0.005;  
        }
        else {
           velocity_constant = 0.02;  
        }
}
...
class moveit_robot {
    public:
      ...
      //  ros::Subscriber gripper_feedback_sub;
       ...
       // void open_gripper();
       // void close_gripper();
        void z_move(double dist1, double dist2, double dist3);
...
void moveit_robot::z_move(double dist1, double dist2, double dist3){
  ...
  //  cout << "Pose:" << move_group.getCurrentPose().pose.position << endl;
   ...
   // cout << "current_end_effector_state:" << getGlobalLinkTransform(end_effector)<< endl;
    ...
    for(int i = 0; i < num_waypoints - 1; i++)      //loop through all waypoints
    {
        ...
        new_timestamp = curr_waypoint->time_from_start.toSec() + (euclidean_distance / 0.5 * velocity_constant);      //start by printing out all 3 of these!
        old_timestamp = next_waypoint->time_from_start.toSec();
    ...
    for(int i = 0; i < num_waypoints; i++)
    {
        ...
        for(int j = 0; j < joint_names.size(); j++)     //loop through all joints in waypoint
        {
            ...
            curr_waypoint->velocities.at(j) = (v1+v2)/2;
            curr_waypoint->accelerations.at(j) = a;
            cout << "curr_waypoint_velocities:" << (v1+v2)/2 << endl; // print velocity values
        }
    }
...
void pick_up_object(moveit_robot &Robot, double down_move_dist1, double down_move_dist2, double down_move_dist3)
{
    ...
    Robot.z_move(down_move_dist1,down_move_dist2,down_move_dist3);
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber subproximity = node_handle.subscribe("distance", 1000, distance_value);
