...
//#if DEBUG      
                ...
                for ( i = 0; i < bytes_read; i++ ) {
                    Serial.print(fifo_data[i]); //daha sonra comment i kaldir
                   Serial.print(" "); // daha sonra comment i kaldir
             }
               // Serial.println(); //daha sonra comment i kaldir
              //  Serial.println(i); //daha sonra comment i kaldir
//#endif
...
//#if DEBUG
               // Serial.print("Down Data: ");
                for ( i = 0; i < gesture_data_.total_gestures; i++ ) {
                   //Serial.print(gesture_data_.r_data[i]);
                   //Serial.print(" ");
                }
                 // Serial.print("");
               // Serial.println(gesture_data_.index++);
                 // Serial.println("");
//#endif
...
#endif
            resetGestureParameters();
             //Serial.println("");  //sonradan eklendi, herbir degeri bir alt satira gecirmek icin
			return motion;
...
/**
 * @brief Gets the Gesture Dimensions
 *
 * Value    Dim
 *   0       both pairs active
 *   1       Up-Down active
 ...
 * @return the current photodiode gain. 0xFF on error.
 */
uint8_t SparkFun_APDS9960::getGestureDim()
{
    uint8_t val;
    
    /* Read value from GCONF3 register */
    if( !wireReadDataByte(APDS9960_GCONF3, val) ) {
        return ERROR;
    }
...
/**
 * @brief Sets the Gesture Dimensions
 *
 * Value    Dim
 *   0       both pairs active
 *   1       Up-Down active
 ...
 * @return True if operation successful. False otherwise.
 */
bool SparkFun_APDS9960::setGestureDim(uint8_t dim)
{
    uint8_t val;
    ...
    /* Write register value back into GCONF3 register */
    if( !wireWriteDataByte(APDS9960_GCONF3, val) ) {
        return false;
    }
