...
void EstimatedForceCallback(const std_msgs::Float32::ConstPtr& msg)
{
//  ROS_INFO("I heard: [%f]", msg->data);

	if( ready_to_write  == true )
		myfileEstimatedForce << msg->data << "\n";
}
...
int main(int argc, char** argv)
{
   ...
   if( argc < 3 )
    ...
    int repetition = std::stoi(argv[3]);		// number of repetition (vertical movements) for exploration of the sensor
    ...
    cout << "[Repetition, Columns] : [" << repetition << ", " << numOfColumns << "]" << endl;
    cout << "===========================================================" << endl;

    cin;

    ros::init(argc, argv, "icm_tactile_sensor_force");
    ...
    ros::Subscriber subEstimatedForce = node_handle.subscribe("estimated_force", 1000, EstimatedForceCallback);
...
//    visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to continue with the next action");
    ...
    targetJoints["shoulder_pan_joint"] = -0.50*3.1416/180;
    targetJoints["shoulder_lift_joint"] = -28.81*3.1416/180;
    targetJoints["elbow_joint"] = 47.61*3.1416/180;
    targetJoints["wrist_1_joint"] = -109.99*3.1416/180;
    targetJoints["wrist_2_joint"] = -90.03*3.1416/180;
    targetJoints["wrist_3_joint"] = 6*3.1416/180;
/*
...
//			myfile.open (folderPath + "/icmSensorData_exp_" + expNumber + "r" + j + "c" + k + ".txt");
            ...
            myfileEstimatedForce.open (folderPath + "/icm_estimatedForce_exp_" + expNumber + "r" + std::to_string(j) + "c" + std::to_string(k) + ".txt");
//            myTextFile.open (folderPath + "/icm_sensor_data_exp_" + expNumber + "r" + std::to_string(j) + "c" + std::to_string(k) + ".txt");

//            if (myTextFile.is_open())
			if (myfilePressure.is_open() && myfileTemperature.is_open() && myfileKalmanX.is_open() && myfileKalmanX.is_open() && myfileAccelerometer.is_open() && myfileGyro.is_open() && myfileForceTorque.is_open() && myfilerobotposz.is_open() && myfileEstimatedForce.is_open() )
				ready_to_write = true;
			else
