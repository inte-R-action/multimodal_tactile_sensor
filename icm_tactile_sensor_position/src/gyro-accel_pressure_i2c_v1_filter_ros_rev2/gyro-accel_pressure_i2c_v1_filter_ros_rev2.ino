...
#include "/home/bathrobot/sketchbook/libraries/ros_lib/multimodal_tactile_sensor/icmDataMsg.h"
...
*/
   ...
   multimodal_tactile_sensor::icmDataMsg sensorData;
   ros::Publisher pub_sensorData("icmSensorData", &sensorData);

   
/*   ros::Publisher pub_rollangle("X_angle", &kal_Angle_X_msg);
   ros::Publisher pub_pitchangle("Y_angle", &kal_Angle_Y_msg);
   ros::Publisher pub_temperature("temperature", &temperature_msg);
   ros::Publisher pub_pressure("pressure", &pressure_msg);
   ros::Publisher pub_loadCell("load_cell", &load_cell_msg);
...
void load_cell_measure(){
  Wire.beginTransmission(load_cell); // set sensor target
  Wire.write(load_cell);   // move your memory pointer to registerAddress
  Wire.endTransmission();  // completes the ‘move memory pointer’ transaction
  Wire.requestFrom(load_cell,2); //request 2 registers (2 bytes)
  load = Wire.read()<<8|Wire.read(); //Store two bytes into load
  output = ((load * 50.0) - 530000.0) / 5783.0;
  load_cell_val = output / 3.0;
  }  
...
void rosData() {
    sensorData.pressure = sensor.getPressurePa()/1000;
/*    sensorData.loadCell = load_cell_val;
    sensorData.temperature = sensor.getTemperatureC();
    sensorData.kalmanX = kalAngleX;
    sensorData.kalmanY = kalAngleY;
...
*/      
    ...
    pub_sensorData.publish(&sensorData);
